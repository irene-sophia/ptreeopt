...
class FugitiveInterception():
    def __init__(self, T, U, R, graph, units_start, fugitive_start, num_sensors, sensor_locations):
        self.T = T
        self.U = U
        self.R = R
        self.graph = graph
    ...
    def f(self, P, mode='optimization'):
        ...
        if mode == 'simulation':
            ...
            for r in range(R):
                ...
                for u in range(U):
                    interception_dict_perroute[f'unit{u}'] = [i for i, j in zip(unit_routes_final[f'unit{u}'], fugitive_routes[r]) if i == j]
        ...
        if mode == 'optimization':
            ...
            for route in range(R):
                interception_pct = 0
                interception_dict = {}
                for u in range(U):
                    interception_dict[f'unit{u}'] = [i for i, j in zip(unit_routes_final[f'unit{u}'], fugitive_routes) if i == j]
