...
class FugitiveInterception():
    def __init__(self, T, U, R, graph, units_start, fugitive_start, num_sensors, sensor_locations, fugitive_routes_db, multiobj=False, seed=1):
        ...
        for node in self.graph.nodes():
            distance_dict[node] = nx.shortest_path_length(G=self.graph,
                                                          source=self.fugitive_start,
                                                          target=node,
                                                          weight='travel_time',
                                                          method='dijkstra')

        nx.set_node_attributes(self.graph, distance_dict, "distance")
        # dictionary to translate nodes to node numbers
        self.nodes_dict = {'node' + str(index): node for index, node in
                           enumerate(sorted(self.graph.nodes(), key=lambda n: self.graph.nodes[n]['distance']))}
    ...
    def f(self, P, mode='optimization'):
        ...
        for r in range(R):
            ...
            for t in range(1, T):
                ...
                units_targetnodes[unit_affected] = self.nodes_dict[target_node]
                try:
                    if units_plan[unit_affected][-1] != target_node:
                        if nx.has_path(G=self.graph, source=units_current[unit_affected], target=self.nodes_dict[target_node]):
                            units_plan[unit_affected] = nx.shortest_path(G=self.graph, source=units_current[unit_affected], target=self.nodes_dict[target_node])
                    ...
                    if nx.has_path(G=self.graph, source=units_current[unit_affected], target=self.nodes_dict[target_node]):
                        units_plan[unit_affected] = nx.shortest_path(G=self.graph, source=units_current[unit_affected],
                                                                     target=self.nodes_dict[target_node])
