    ...
    for rep in range(num_repetitions):
        ...
        for seed_rep in range(num_seeds):
            ...
            print('finished seed rep ', seed_rep+1, 'of repetition ', rep+1, '(experiment number ',
                  (num_repetitions*rep*num_seeds)+(num_seeds*seed_rep)+1, 'of ', (num_seeds*num_repetitions), ').')
            #results_df, success = model.f(best_solution, mode='simulation')
            #print('Simulation: interception percentage: ', (sum(success.values()) * 100)/R)  # re-initializes!! baaaad: only use for visualization
